...
| [Chime-in](../cogs/chimein.py) | silent | up to **10/day**, **40 min** cooldown, score **>= 0.8** | up to **20/day**, **20 min** cooldown, score **>= 0.6** |
...
- **On/off + cadence:** the discourse mood. `mood=off` silences chime-in; `chill` makes her reserved (10/day, 40 min cooldown, 0.8 threshold); `yaps` makes her chatty (20/day, 20 min cooldown, 0.6 threshold).
...
2. **`tasks.loop(seconds=60)` tick** refreshes each guild's listen channel from settings, then walks every (guild, channel) with new buffered activity and runs the gate sequence in `_maybe_chime_in_one()`:
   ...
   - **daily_cap_gate**: bounded by the mood-tuned daily cap (10 chill / 20 yaps) per channel per 24h
